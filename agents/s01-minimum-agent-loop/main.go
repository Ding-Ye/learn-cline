...
// providerProfiles — convenient shortcuts for the popular OpenAI-compatible
// endpoints (and Anthropic's native API). Pass `-provider <name>` and we fill
// in the base URL and a default model. Override anytime with `-base-url` /
// `-model`. This is s01's tiny echo of cline's buildApiHandler factory
// (apps/vscode/src/core/api/index.ts), which picks a handler per provider.
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(),
			"usage: s01 [-v] [-provider P] [-base-url URL] [-model ID] <task prompt>\n\n"+
				"  The minimum agent loop: send the conversation, run the tool the model\n"+
				"  asks for, feed the result back, repeat until it stops.\n\n"+
				...
				"  Examples:\n"+
				"    s01 -v \"write hello.txt with the text hi\"        # default Anthropic\n"+
				"    s01 -provider deepseek -v \"run: echo hi\"          # DeepSeek\n"+
	...
	// Tools run against the current working directory. write_file is confined
	// there; bash inherits it.
	cwd, _ := os.Getwd()
	tools := []Tool{NewWriteFileTool(cwd), NewBashTool()}
	...
	final, err := task.Run(context.Background(), prompt)
	if err != nil {
